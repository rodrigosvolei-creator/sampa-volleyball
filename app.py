...
def _build_jogos_from_grupos(grupos, fmt, is_test=False, hora_inicio="08:30", intervalo_min=75):
    ...
    # 3) Adiciona semis (a serem preenchidas depois) — vão na ordem definida
    semis_finais = []
    if fmt == "hexagonal":
        semis_finais.append({"id": str(uuid.uuid4())[:8], "fase": "semi", "label": "Semi 1: 1º x 4º",
            "equipe_a": None, "equipe_b": None, "sets_a": 0, "sets_b": 0,
            "parciais": [], "finalizado": False, **base})
        semis_finais.append({"id": str(uuid.uuid4())[:8], "fase": "semi", "label": "Semi 2: 2º x 3º",
            "equipe_a": None, "equipe_b": None, "sets_a": 0, "sets_b": 0,
            "parciais": [], "finalizado": False, **base})
    else:
        semis_finais.append({"id": str(uuid.uuid4())[:8], "fase": "semi", "label": "Semi 1: 1ºA x 2ºB",
            "equipe_a": None, "equipe_b": None, "sets_a": 0, "sets_b": 0,
            "parciais": [], "finalizado": False, **base})
        semis_finais.append({"id": str(uuid.uuid4())[:8], "fase": "semi", "label": "Semi 2: 1ºB x 2ºA",
            "equipe_a": None, "equipe_b": None, "sets_a": 0, "sets_b": 0,
            "parciais": [], "finalizado": False, **base})
    semis_finais.append({"id": str(uuid.uuid4())[:8], "fase": "terceiro", "label": "Disputa 3º Lugar",
        "equipe_a": None, "equipe_b": None, "sets_a": 0, "sets_b": 0,
        "parciais": [], "finalizado": False, **base})
    semis_finais.append({"id": str(uuid.uuid4())[:8], "fase": "final", "label": "Final",
        "equipe_a": None, "equipe_b": None, "sets_a": 0, "sets_b": 0,
        "parciais": [], "finalizado": False, **base})
    # 4) Concatena: fase regular (reordenada) → semis → 3º → final
    todos = fase_regular + semis_finais
