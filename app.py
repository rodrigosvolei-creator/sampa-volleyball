...
def auto_classify_semis(data, naipe):
    ...
    # === Auto-fill final and 3rd place when both semis are done ===
...
def track_visit(resp, page):
    """Track page visit with unique visitor cookie"""
    data = load_data()
    ...
    if visitor_id not in day_data.get("visitors", []):
        day_data["unicos"] += 1
        if len(day_data["visitors"]) < 5000:  # limit stored IDs
            day_data["visitors"].append(visitor_id)
    # Clean old visitor lists (keep only last 7 days)
...
def upload_logo_equipe(naipe, equipe_id):
    ...
    filename = f"logo_equipe_{naipe}_{equipe_id}.{ext}"
    filepath = os.path.join(UPLOADS_DIR, filename)
    # Save and resize to 200x200
    try:
        img = Image.open(file.stream)
        img = img.convert("RGBA" if ext == "png" else "RGB")
        img.thumbnail((200, 200), Image.LANCZOS)
        img.save(filepath, "PNG" if ext == "png" else "JPEG", quality=85)
    except:
        file.seek(0)
        file.save(filepath)
    data = load_data()
...
def get_galeria():
    """Returns all photos: team photos + MVP highlights + general gallery"""
    data = load_data()
    galeria = []
