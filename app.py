...
def load_data():
    ...
    if os.path.exists(DATA_FILE):
        with open(DATA_FILE, 'r', encoding='utf-8') as f:
            ...
            # Ensure config exists for both naipes
...
def get_config_route(naipe):
    data = load_data()
    return jsonify(get_config(data, naipe))
...
def sortear_grupos(naipe):
    ...
    if fmt == "hexagonal":
        # Hexagonal: all teams in one group "A", B empty
        data["grupos"][naipe] = {"A": ids, "B": []}
    else:
        # Grupos: split into 2 groups
        half = len(ids) // 2
...
def gerar_jogos(naipe):
    ...
    if fmt == "hexagonal":
        # All teams in grupo A, round-robin
        eids = grupos.get("A", [])
        for i in range(len(eids)):
            for j in range(i + 1, len(eids)):
                jogos.append({
                    "id": str(uuid.uuid4())[:8], "fase": "hexagonal", "grupo": "",
                    "equipe_a": eids[i], "equipe_b": eids[j],
                    "sets_a": 0, "sets_b": 0, "parciais": [], "finalizado": False
                })
        # Semis: 1º x 4º, 2º x 3º
        ...
        # Grupos: round-robin per group
        for gn, eids in grupos.items():
            for i in range(len(eids)):
                for j in range(i + 1, len(eids)):
                    jogos.append({
                        "id": str(uuid.uuid4())[:8], "fase": "grupos", "grupo": gn,
                        "equipe_a": eids[i], "equipe_b": eids[j],
                        "sets_a": 0, "sets_b": 0, "parciais": [], "finalizado": False
                    })
    ...
    # Both formats get final + 3rd place
...
def get_classificacao(naipe, grupo):
    ...
    # Filter games: hexagonal uses "hexagonal" fase, grupos uses "grupos"
    ...
    for j in jogos:
        if j.get("fase") != fase_filter or not j.get("finalizado"):
            continue
        # For grupos format, also filter by grupo name
