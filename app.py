...
def auto_classify_semis(data, naipe):
    ...
    # Jogos de teste nunca participam de auto-classificação
    ...
    if len(semis) == 2 and all(s.get("finalizado") for s in semis):
        ...
        for j in jogos:
            if j["fase"] == "final" and not j.get("equipe_a"):
                ...
                j["equipe_a"] = loser1
                j["equipe_b"] = loser2
    # Merge jogos modificados de volta no data, preservando jogos de teste (is_test) intactos
    by_id = {j["id"]: j for j in jogos}
    merged = []
    for j in data["jogos"].get(naipe, []):
        if j.get("is_test"):
            merged.append(j)  # Teste nunca foi tocado, preserva
        elif j["id"] in by_id:
            merged.append(by_id[j["id"]])
        else:
            merged.append(j)  # Não estava na lista filtrada por algum motivo
    ...
    def _do(data):
        ...
        if "max_equipes" in body:
            try:
                ...
                pass
        if "formato_jogos" in body and body["formato_jogos"] in ("hexagonal", "grupos"):
...
def get_classificacao(naipe, grupo):
    ...
    relevant_jogos = [j for j in jogos
                      ...
                      and (fmt == "hexagonal" or j.get("grupo") == grupo)
...
@admin_required
def test_state():
    """Retorna estado atual do ambiente de teste (existe? qual o jogo?)."""
    data = load_data()
    test_equipes = []
    for naipe in ("masculino", "feminino"):
        for e in data["equipes"].get(naipe, []):
    ...
    for naipe in ("masculino", "feminino"):
        for j in data["jogos"].get(naipe, []):
            if j.get("is_test"):
                ...
                test_jogo = {
                    ...
                    "equipe_a_nome": emap.get(j.get("equipe_a"), "?"),
                    "equipe_b_nome": emap.get(j.get("equipe_b"), "?"),
        ...
        if test_jogo:
            break
    return jsonify({
        "active": bool(test_equipes and test_jogo),
...
@admin_required
def test_jogo():
    """Retorna o jogo de teste completo (com todos os campos), pra UI admin abrir o modal Ao Vivo."""
    data = load_data()
    for naipe in ("masculino", "feminino"):
        for j in data["jogos"].get(naipe, []):
            if j.get("is_test"):
...
@admin_required
def test_setup():
    """Cria/recria ambiente de teste: 2 equipes-fantasma + 1 jogo entre elas, todos is_test=true."""
    naipe = TEST_NAIPE
    def _do(data):
        # Limpa qualquer ambiente de teste anterior
        ...
        # Cria 2 equipes de teste
        eq_a = {
            "id": "test_" + uuid.uuid4().hex[:6],
            "nome": "🧪 Time Teste A",
            "responsavel": "Modo Teste",
            "telefone": "",
            "login": "teste_a_" + uuid.uuid4().hex[:4],
        ...
        eq_b = {
            ...
            "responsavel": "Modo Teste",
            "telefone": "",
            "login": "teste_b_" + uuid.uuid4().hex[:4],
            "senha_hash": generate_password_hash("teste123"),
            "pagamento_status": "aprovado",
            "comprovante": None,
            "created_at": datetime.now().isoformat(),
        ...
        # Cria 1 jogo de teste entre elas
        jogo = {
            "id": "test_" + uuid.uuid4().hex[:6],
            "fase": "teste",  # Fase especial não-padrão pra não cair em filtros de classificação/auto-classify
            "grupo": "",
...
@admin_required
def test_teardown():
    """Remove TODO o ambiente de teste (equipes is_test + jogos is_test em ambos os naipes)."""
    ...
    def _do(data):
        for naipe in ("masculino", "feminino"):
            ...
            test_jogos_count = sum(1 for j in data["jogos"].get(naipe, []) if j.get("is_test"))
            data["jogos"][naipe] = [j for j in data["jogos"].get(naipe, []) if not j.get("is_test")]
            counts["jogos"] += test_jogos_count
            # Remove atletas de equipes de teste (caso alguém tenha cadastrado)
